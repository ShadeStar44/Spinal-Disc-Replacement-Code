...
% and the scale factor% Input: E, F - lists of 2D corresponding points. These are n by 2 matrices
%               where n is the number of points and the x val is in column 1
%               y val in column 2.% Output: fcr - the finite centre of rotation.
%       theta - the angle of rotation between the two sets of point
%       about the fcr.
...
[rF, cF] = size(F);
if (cE~=2)||(cF~=2)
    error('E, F should be an nx 2matrix')
end
if (rE~= rF)
...
meanA = sum(A)/rE;
meanB = sum(B)/rF;
% now translate both pointsets to the originA 
A2= A - meanA;
B2= B - meanB;
